# Prompt Quick Reference

| Situation | Prompt |
|---|---|
| New vague feature request | `prompts/features/autonomous-feature-delivery.md` |
| Orient to unfamiliar repo | `prompts/codebase/understand-repository.md` |
| Design before coding | `prompts/architecture/design-new-feature.md` |
| Bug | `prompts/debugging/*` + `evidence-first-debug.md` |
| Tests | `prompts/testing/*` |
| Security pass | `prompts/security/full-security-review.md` |
| PR / diff review | `prompts/code-review/full-review.md` |
| UI polish | `prompts/ui/ui-polish.md` |
| Deploy anxiety | `prompts/deployment/production-readiness.md` |
| Time crunch | `prompts/hackathon/enter-mode.md` |
| Split team work | `prompts/hackathon/team-split.md` |
| Protect live demo | `prompts/hackathon/protect-demo.md` |

Always also follow `AGENTS.md`.
| Learn web fundamentals | `prompts/features/web-fundamentals-tutor.md` + `docs/training/web-fundamentals/` |
| Hackathon ideation | `prompts/hackathon/decode-prompt.md` → `ideate-twenty.md` → `decide-mvp.md` |
| Hackathon execution / pitch | `prompts/hackathon/execution-lead.md`, `pitch-critique.md` |
